	...
	const fetchProduct = async () => {
		try {
			const [_product, _productInfo] = await Promise.all([
				axios.get(
					`${process.env.NEXT_PUBLIC_API_URL}/user/products/${productId}`
				),
				axios.get(
	...
	return (
		<main className='w-full relative'>
			...
			<div className='flex flex-col justify-start items-start py-10 md:px-8'>
				...
				<div className='grid grid-cols-2 gap-5 md:gap-5 md:flex items-center justify-start w-full mt-10 rounded-lg px-4 md:px-0'>
					{product?.media?.slice(0, 6)?.map((media, index) => (
						<div
							key={media.id}
